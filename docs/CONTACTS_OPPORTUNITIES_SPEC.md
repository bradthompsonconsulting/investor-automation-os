...
**Tags, pipeline stage, `offer_` fields, workflow triggers.** This is unchanged from `CONTACT_WORKSPACE_SPEC_v2.md` §4 and is NOT relaxed by any write class below. Editing a contact field is not a licence to touch a tag, move a stage, or set an offer field. IAOS writes reach GHL workflows only through reviewed, intentional inputs: Board #4's reviewed disposition and routing fields, the separately governed scoring tags (hot/warm/low; `INV95_WRITE_BOUNDARIES.md`, root motivation-score), and the single guarded stage exception in §4.1a, whose only permitted entry workflow is the named, reviewed `Seller - Under Contract Exit`. The §4.1a exception permits no other workflow enrollment.
...
The transition permits no workflow enrollment other than the one named, reviewed entry workflow in P1 (`Seller - Under Contract Exit`).

**Production prerequisites — both required before the first Production transition:**

- **P1 — Entry audit and the one permitted entry workflow (amended 2026-09-24).** Only the named, reviewed `Seller - Under Contract Exit` may fire on entry into Under Contract. Any other published workflow that fires on entry — a pipeline-stage-changed trigger filtered to Under Contract or unfiltered, an opportunity-changed or status trigger, or any other trigger the move satisfies — blocks activation.
  - **Reviewed design (Test).** Trigger: pipeline stage changed, scoped to Seller Leads Pipeline / Under Contract / status Open. Action: one Remove action targeting Seller 6, Seller 7 and Seller 8. It moves no stage and sets no tag or field. Any change to its trigger or action requires a new review.
  - **Production reconfirmation.** Spock has performed a read-only audit of the Production location's GHL workflow builder (triggers are not API-derivable, §4.6); it is not stored in this repository. After the Production Under Contract stage is provisioned, and before the first Production transition, a read-only builder reconfirmation must find that `Seller - Under Contract Exit` exists in Production with exactly this reviewed design and that no other published workflow fires on entry. The reconfirmation is recorded in the Board #9 activation evidence.
  - **Test evidence (Test location only).** Directly observed: on 2026-09-24 at 12:18 AM CDT, a synthetic move into Under Contract fired the Test `Seller - Under Contract Exit` (workflow `0be799c6-1c0a-40be-a0eb-38b6f8f5247d`), which removed a Seller 6 run that was mid-Wait; the Seller 6 log reads "Removed by - External workflow action" and that enrollment Finished. Inferred, not directly proven: removal from Seller 7 and Seller 8, from the shared Remove action. This is an execution proof of the Remove action, not a Test entry-trigger audit, and Test evidence does not establish Production behavior.
  - **Production proof.** A separately authorized, supervised Production proof is required before the first real Production Under Contract transition. Its procedure, acceptance criteria and safety boundaries require approval before it is executed. This section prescribes no enrollment method, and it does not treat failed or skipped messages as harmless. The no-outbound-attempt rule is the current Test proof constraint; this section does not define it as a Production rule, and this documentation authorizes no outbound attempt.
- **P2 — Exit guards (not built, not proven; amended 2026-09-24).** Five exit-risk workflows can move an existing opportunity out of Under Contract without an Under Contract guard:
  - **Find-based opportunity selection:** `Seller - Follow Up`, `Seller - Not Interested` and `Seller - Route to Long-Term Nurture` each find a Seller Leads opportunity and update its stage (Spock inspected the Production Find → Update path of `Seller - Route to Long-Term Nurture`).
  - **Opportunity-context paths:** `Seller 6` and `Seller 7` each have a path that can move the opportunity already in context to Long-Term Nurture.
...
- **Each `Phone N DNC` field renders adjacent to its `Phone N` field** — the DNC flag sits with the number it qualifies. This governs **`Phone 2 DNC`–`Phone 5 DNC`**, which sit beside `Phone 2`–`Phone 5` in the **Reachability** subgroup. **`Phone 1 DNC` is the exception:** there is no `Phone 1` custom field (the primary phone is a native identity field, not among the 96), and — because **line 156 governs the primary phone's placement** (§4-v3 Class 2 / §4.3 governs its treatment, not its render position) — `Phone 1 DNC` renders **in the `Additional Info` → Reachability subgroup** — it IS an `Additional Info` custom field (`contact.phone_1_dnc`), and the render buckets it there by its config subgroup mapping. **SUPERSEDED (2026-07-24, D1):** the earlier "identity block, adjacent to the native primary phone" placement is void — D1 (`CONTACTS_DETAIL_SPEC.md`) reuses the ContactWorkspace header (name / formatted primary phone / combined address), which has no `Phone 1 DNC` slot. The "no phone number renders without its DNC status" intent holds for `Phone 2`–`Phone 5`; `Phone 1 DNC` sits first-by-position (900) in Reachability.
- **Identity header renders FIRST (D1) — the reused ContactWorkspace header is the top section of the `/contacts/:id` form, above all six folder sections.** **SUPERSEDED (2026-07-24, D1 — `CONTACTS_DETAIL_SPEC.md`):** the earlier "native identity block of six separate fields (primary phone, primary email, `address1`/`city`/`state`/`postalCode`) with `Phone 1 DNC` inside it" is void. D1 reuses the existing header, which renders exactly **contact-name `<h1>`, formatted primary phone (`formatPhone`), and combined address (`formatAddress` = `address1` + `city`/`state`/`postalCode` joined)** — **no primary email, no field-by-field address, no `Phone 1 DNC`** (that field renders in the Reachability subgroup, per the line above). Their **treatment** — read-only in edit, dedup anchors — remains governed by §4-v3 Class 2 (§4.3).
...
- **Edit affordances are ABSENT in Phase A, not disabled** — render read-only text, no input elements. A greyed-out input the reader cannot distinguish from a wired one that failed is not an acceptable intermediate state. **SCOPED (2026-07-24, D1):** "no input elements" scopes to the **Phase A field display** — the six-folder record-section container plus the D1 identity header — NOT the whole `/contacts/:id` route. Under D1 the route also hosts the cohabiting Workspace features (§3): the always-present notes `<input>` and the callback `datetime-local` inside `CallbackPopover`. Those are out of scope for the Phase A field display; `<button>` disclosures do not count as inputs.
...
- **Phase A floor — read assertions only.** It covers: the grid renders; search / filter / sort behave (**V1 has no filters, so the "filter" assertion asserts ABSENCE of any filter control** — a positive filter-behaves check when filters land; §5.1 Grid layout); the Phone column formats a known `+1`+10-digit fixture as `214-914-6151` (DISPLAY-only, raw value unaffected — §5.1 phone display); the detail view renders all **96** custom fields in **folder-then-subgroup-then-`position`** order for `Additional Info` (its four subgroups in the order Reachability, Property, Investor, System, each internally by GHL `position` — NOT raw `position` across all 73) and **folder-then-`position`** order for every other folder; the **D1 identity header renders — contact-name `<h1>`, formatted primary phone, and combined address — as THREE assertions** (address is one combined render per D1, NOT field-by-field; primary email is NOT rendered; the contact-name check is **NEW COVERAGE** added at the 2026-07-24 D1 amend — strict D1 reconciliation of the original six was **2**, name is a deliberate **+1**); the **four `Additional Info` subgroups** render; **`Phone 2 DNC`–`Phone 5 DNC` render adjacent to their `Phone N` in the Reachability subgroup**; and **no input/textarea/select/contenteditable exists within the Phase A field display** — the six-folder record-section container plus the D1 identity header (**SCOPED 2026-07-24, D1:** NOT the whole `/contacts/:id` route, which also hosts the cohabiting Workspace notes `<input>` and the `CallbackPopover` `datetime-local` per §3; `<button>` disclosures do not violate it). **Each of the 96 field assertions checks three things in ONE `check()`: (1) PRESENCE; (2) CORRECT SECTION — its folder, and for `Additional Info` its subgroup; and (3) RELATIVE ORDER within that section — NOT an absolute index. `Phone 1 DNC` asserts as: present in the `Additional Info` → Reachability subgroup (its field-check among the 96).** Cross-section ordering is NOT a per-field absolute ordinal — it moves into checks already counted in the floor: **the 6 folder-section checks and the 4 `Additional Info` subgroup checks now assert each section's RENDERED POSITION IN SEQUENCE, not merely its presence.** **RATIONALE (this reverses the a4208a2 "GLOBAL index 1..96" definition):** absolute ordinals carry no business meaning, and they make a harmless section reorder look like ~95 regressions; relative-within-section order plus section-sequence checks localize a real defect to its section without that false-positive blast radius. That relative-order change did not move the floor — same check counts, richer assertions. The floor moved 122 → 123 separately, on 2026-07-23, when the phone-format display check was added.
...
**Phase A floor integer — 119.** Enumerated as `check()` calls on the happy path: grid + search + filter + sort + phone-format (5) + six folder sections (6) + the 96 custom fields (96) + four `Additional Info` subgroups (4) + three D1 identity-header renders — contact-name, formatted primary phone, combined address (3; strict D1 reconciliation was 2, contact-name is a deliberate +1 NEW COVERAGE) + four `Phone N DNC` adjacencies — `Phone 2 DNC`–`Phone 5 DNC` beside their `Phone N` in Reachability (4) + no-input-element — **SCOPED (2026-07-24, D1)** to the Phase A field display (record section + D1 identity header, NOT the whole route; the cohabiting Workspace notes/callback inputs are out of scope per §3) (1) = **119**. The **phone-format** check asserts a known `+1`+10-digit fixture row renders in the Phone column as `214-914-6151` (DISPLAY-only; raw value unaffected — §5.1 phone display); it is the single increment over the original 122, added 2026-07-23 after phone formatting shipped (`419ab12`/`c502f90`) but still BEFORE the harness is coded — never back-filled from a passing run.

**Unexercised branches (not defects, not pending work):** the null-`dateAdded` sort branch and the non-empty malformed-phone display fallback are NOT live-coverable under the current Phase A production-only harness constraints. OBSERVED 2026-07-23 against 42 live rows: 42/42 have `dateAdded` (GHL stamps it at creation) and 0 rows are non-empty non-US/malformed. A passing 119/119 run therefore does NOT mean these branches were exercised. Synthetic fixtures, a test location, or a later local/mock harness could cover them; none exists today.
...
_Narrative §0–3 (per Brad) and §5 (surface design & build: layout / build order / verification / render-config data source) drafted 2026-07-22 (§5.4 render-config source & runtime failure contract added 2026-07-23); the §5.3 Phase A verification floor integer is 119 (122 → 123 on 2026-07-23 by the phone-format check, then 123 → 119 on 2026-07-24 reconciling the identity category to D1)._
