...
`docs/CONTACTS_OPPORTUNITIES_SPEC.md` now holds **§0–3 (narrative) DRAFTED** (`c6ae880`) + **§4-v3 write invariant** (`cc06568`) + the recon-findings log; **§5+ undrafted.** This is the first surface that EDITS contact fields — a materially larger write surface than every prior (read-only or three-write) phase.
...
- **§0–3 narrative DRAFTED (`c6ae880`):**
  ...
  - **Identity fields READ-ONLY in edit (per §4.3): correction = CREATE A NEW CONTACT**, do not edit. **No cleanup of the superseded record needed** — a bad number washes out via call disposition; a good number reaching the wrong person resolves through normal lead work.
...
- **NEXT WORK:** **§5+ (layout, build order, verification) undrafted.** Fixture proofs (Class 1 per-field inert, Class 2 additionals collision-test, Class 3 create inert, element-shape) remain **NOT started** — still gated to build time.
