...
- **NEXT WORK:** read path is now known, so the next build step is **drafting §0–3 (the read view)** of `CONTACTS_OPPORTUNITIES_SPEC.md`. Fixture proofs (Class 1 per-field inert, Class 2 additionals collision-test, Class 3 create inert, element-shape) remain **NOT started** — still gated to build time.
