/**
 * INV-67 checkbox-marker / broker-model repair -- Batch 3 of 3. Creates
 * the 22 narrowly-scoped Opportunity custom TEXT fields
 * `src/lib/contract-checkbox-marker-model.ts`'s `BROKER_TEXT_KEYS`
 * requires -- TREC 20-19 page 11's Seller-broker and Buyer-broker blocks
 * (11 printed destinations per side). Mirrors Batch 1's
 * (`inv67-create-checkbox-marker-fields-batch1.cjs`) and Batch 2's
 * (`inv67-create-checkbox-text-fields-batch2.cjs`) hardened architecture
 * exactly -- neither of those scripts is modified by this one; this is a
 * new, sibling script for a third, different 22-key spec set.
 *
 ...
 * resolution, the complete unfiltered 22-spec batch always preflighted
 ...
 * to Batch 1/2's, applied to this batch's own 22 specs.
 *
 * PAGE-11 MODEL BOUNDARIES (locked, this repair's V1 scope):
 *   - These 22 fields populate ONLY the separate Seller-broker and
 *     Buyer-broker blocks -- no intermediary fields exist here or
 *     anywhere in this repair; intermediary arrangements are unsupported
 *     and fail closed before a Contract Draft Request
 *     (`contract-broker-arrangement-model.ts`).
 *   - No city/state/ZIP split -- TREC's own page 11 has ONE `Address`
 *     blank per side; this script creates exactly that, nothing more.
 *   - Team Name and Licensed Supervisor Phone are both included -- the
 *     two fields an earlier, corrected plan draft had wrongly omitted.
 *   - This script does not model or touch paragraph 8 broker-conflict
 *     disclosure in any way -- that is explicitly out of this repair's
 *     scope.
 *
 ...
 *
 * Usage:
 *   node scripts/inv67-create-broker-text-fields-batch3.cjs --location <id>
 *                                                            --credential-file <path>
 *                                                            [--apply]
 *
 * There is NO `--only` flag -- the complete, unfiltered 22-spec batch is
 * always preflighted together, exactly like Batch 1/2.
 */
const fs = require('fs');
 ...
 * `BROKER_TEXT_KEYS` exactly -- same 22 keys, same order (11 seller-side
 * suffixes, then 11 buyer-side suffixes, `BROKER_FIELD_SUFFIXES` order
 * preserved on both sides). Verified programmatically at the top of
 * `main()` (throws before any network call if the two ever diverge)
 * rather than trusted by eye alone. Display names are PROPOSED, not yet
 * approved -- deterministic, "Contract Seller Broker ..." / "Contract
 * Buyer Broker ..." prefixed, one pair per TREC page-11 printed
 * destination (Broker Firm, Address, Broker Firm License No., Associate
 * Name, Team Name, Associate Email, Associate Phone, Associate License
 * No., Licensed Supervisor Name, Licensed Supervisor Phone, Licensed
 * Supervisor License No.) -- no city/state/ZIP, no intermediary field.
 */
const FIELD_SPECS = [
  { key: 'seller_broker_firm_name_text', name: 'Contract Seller Broker Firm Name' },
  { key: 'seller_broker_address_text', name: 'Contract Seller Broker Address' },
  { key: 'seller_broker_firm_license_no_text', name: 'Contract Seller Broker Firm License No' },
  { key: 'seller_broker_associate_name_text', name: 'Contract Seller Broker Associate Name' },
  { key: 'seller_broker_team_name_text', name: 'Contract Seller Broker Team Name' },
  { key: 'seller_broker_associate_email_text', name: 'Contract Seller Broker Associate Email' },
  { key: 'seller_broker_associate_phone_text', name: 'Contract Seller Broker Associate Phone' },
  { key: 'seller_broker_associate_license_no_text', name: 'Contract Seller Broker Associate License No' },
  { key: 'seller_broker_supervisor_name_text', name: 'Contract Seller Broker Supervisor Name' },
  { key: 'seller_broker_supervisor_phone_text', name: 'Contract Seller Broker Supervisor Phone' },
  { key: 'seller_broker_supervisor_license_no_text', name: 'Contract Seller Broker Supervisor License No' },
  { key: 'buyer_broker_firm_name_text', name: 'Contract Buyer Broker Firm Name' },
  { key: 'buyer_broker_address_text', name: 'Contract Buyer Broker Address' },
  { key: 'buyer_broker_firm_license_no_text', name: 'Contract Buyer Broker Firm License No' },
  { key: 'buyer_broker_associate_name_text', name: 'Contract Buyer Broker Associate Name' },
  { key: 'buyer_broker_team_name_text', name: 'Contract Buyer Broker Team Name' },
  { key: 'buyer_broker_associate_email_text', name: 'Contract Buyer Broker Associate Email' },
  { key: 'buyer_broker_associate_phone_text', name: 'Contract Buyer Broker Associate Phone' },
  { key: 'buyer_broker_associate_license_no_text', name: 'Contract Buyer Broker Associate License No' },
  { key: 'buyer_broker_supervisor_name_text', name: 'Contract Buyer Broker Supervisor Name' },
  { key: 'buyer_broker_supervisor_phone_text', name: 'Contract Buyer Broker Supervisor Phone' },
  { key: 'buyer_broker_supervisor_license_no_text', name: 'Contract Buyer Broker Supervisor License No' },
].map((s) => ({ ...s, dataType: 'TEXT' }));

/**
 * Fails loud, before any network call, if this script's own FIELD_SPECS
 * ever drifts from the authoritative `BROKER_TEXT_KEYS`. Reads the
...
function verifyAgainstAuthoritativeSource() {
  ...
  const m = src.match(/export const BROKER_TEXT_KEYS = \[([\s\S]*?)\] as const;/);
  if (!m) die(`could not find BROKER_TEXT_KEYS in ${srcPath} -- refusing to run against a source this script cannot verify.`);
  // BROKER_TEXT_KEYS is computed from BROKER_FIELD_SUFFIXES.map(...), not a
  // literal string array like CHECKBOX_MARKER_KEYS/CHECKBOX_TEXT_KEYS -- so
  // derive the same 22 keys the SAME way the source does, from the same
  // suffix list, rather than trying to regex-extract a computed expression.
  const suffixMatch = src.match(/export const BROKER_FIELD_SUFFIXES = \[([\s\S]*?)\] as const;/);
  if (!suffixMatch) die(`could not find BROKER_FIELD_SUFFIXES in ${srcPath} -- refusing to run against a source this script cannot verify.`);
  const suffixes = suffixMatch[1]
    .split(/\r?\n/)
    .map((l) => l.match(/^\s*"([^"]+)",?\s*$/))
    .filter(Boolean)
    .map((mm) => mm[1]);
  if (suffixes.length !== 11) die(`authoritative BROKER_FIELD_SUFFIXES has ${suffixes.length} entries, expected 11 -- STOP, source has drifted since this script was written.`);
  const authoritative = [...suffixes.map((s) => `seller_broker_${s}_text`), ...suffixes.map((s) => `buyer_broker_${s}_text`)];
  const proposed = FIELD_SPECS.map((s) => s.key);
  if (authoritative.length !== 22) die(`derived BROKER_TEXT_KEYS has ${authoritative.length} keys, expected 22 -- STOP.`);
  if (JSON.stringify(authoritative) !== JSON.stringify(proposed)) {
    ...
    die(
      `FIELD_SPECS has drifted from BROKER_TEXT_KEYS (derived from BROKER_FIELD_SUFFIXES). missing=${JSON.stringify(missing)} extra=${JSON.stringify(extra)} -- ` +
      `refusing to run. Update this script's FIELD_SPECS to match the authoritative source exactly before retrying.`,
    );
  }
  console.log(`Verified: this script's 22 FIELD_SPECS keys match BROKER_TEXT_KEYS exactly (same 22, same order, derived from the same 11 BROKER_FIELD_SUFFIXES on both sides).\n`);
}

/**
 * The ONE canonical anchor this script resolves its target folder from --
 * identical discipline to Batch 1/2: the anchor must exist exactly once
 * and carry a non-empty `parentId`, or this script refuses before any
 * POST. No fallback to an arbitrary folder.
 */
 ...
 * identical logic to Batch 1/2's `classifyExistingMatch`. `{kind:'none'}`
 ...
 * Batch 1/2: `model` and `parentId` are checked strictly, confirmed live
 * (Batch 1/2 sessions) that GHL's customFields list AND single-field GET
...
/**
 * Parses a POST response body defensively -- identical to Batch 1/2's
...
/**
 * Pure preflight over ALL 22 specs at once -- classifies every one before
...
async function main() {
  ...
  function printSummaryAndExit(code, closingMessage) {
    ...
    for (const r of rows) {
      console.log(`${String(r.ordinal).padStart(2)}. ${r.key.padEnd(45)} | ${r.name.padEnd(45)} | ${r.fieldKey.padEnd(55)} | ${r.mergeTag.padEnd(60)} | ${r.detail}`);
    }
