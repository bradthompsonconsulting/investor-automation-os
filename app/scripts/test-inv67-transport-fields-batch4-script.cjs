/**
 * INV-67 compound text-destination repair -- Batch 4 GHL Test provisioning
 * script safety proof. Proves `inv67-create-transport-only-fields-batch4.cjs`
 * satisfies every safety property required before it is authorized for a
 * live `--apply` run, WITHOUT ever making a network call itself -- the
 * same properties every prior batch's script and suite established,
 * applied to this batch's own 4 `CONTRACT_PROJECTION_TRANSPORT_ONLY_KEYS`
 * specs:
 *
 *   1. Exactly 4 FIELD_SPECS, exactly equal (same 4, same order) to the
 *      authoritative `CONTRACT_PROJECTION_TRANSPORT_ONLY_KEYS`.
 ...
 *   8. `planBatch` -- preflights ALL 4 specs before any POST; one conflict
 *      refuses the WHOLE batch. A future successful apply, re-run, would
 *      classify all four as reuse -- never a duplicate create.
 ...
 *       exiting.
 *   13. This script does NOT modify the Seller Count script, and declares
 *       exactly the 4 authorized keys -- no Contract Seller Count, marker,
 *       broker, or retired field is included.
 */
...
const SCRIPT = path.join(APP, 'scripts', 'inv67-create-transport-only-fields-batch4.cjs');
const SELLER_COUNT_SCRIPT = path.join(APP, 'scripts', 'inv67-create-seller-count-field.cjs');
const TMP = path.join(APP, '.tmp-inv67-batch4-script-test');
...
try {
  execSync(`npx tsc "${path.join(APP, 'src', 'lib', 'contract-ghl-projection-model.ts')}" --outDir "${TMP}" --module commonjs --target es2020 --strict`, { cwd: APP, stdio: 'inherit' });
} catch (_) {
  console.error('ABORT: TypeScript compilation failed. Nothing tested.');
  cleanup();
  process.exit(10);
}
const { CONTRACT_PROJECTION_TRANSPORT_ONLY_KEYS } = require(path.join(TMP, 'contract-ghl-projection-model.js'));
...
function realField(overrides) {
  return { id: 'existing-id-1', name: 'Contract Additional Earnest Money Amount', fieldKey: 'opportunity.contract_additional_earnest_money_amount', dataType: 'TEXT', model: 'opportunity', parentId: PARENT_ID, ...overrides };
}
const SPEC1 = S.FIELD_SPECS[0]; // additional_earnest_money_amount_text
const SPEC1_KEY = S.expectedFieldKey(SPEC1.name);

/* ==================================================================== */
/* 1. FIELD_SPECS -- exactly 4, exactly equal to CONTRACT_PROJECTION_    */
...
check(
  ...
  S.FIELD_SPECS.map((s) => s.name),
  [
    'Contract Additional Earnest Money Amount',
    'Contract Additional Earnest Money Days',
    'Contract Closing Date Month Day',
    'Contract Closing Date Year Suffix',
  ],
);
check(
  'the exact authorized expected fieldKeys, in order',
  S.FIELD_SPECS.map((s) => S.expectedFieldKey(s.name)),
  [
    'opportunity.contract_additional_earnest_money_amount',
    'opportunity.contract_additional_earnest_money_days',
    'opportunity.contract_closing_date_month_day',
    'opportunity.contract_closing_date_year_suffix',
  ],
);
checkTrue('Contract Seller Count is NOT one of the FIELD_SPECS names (the doc comment may reference it by name only to document the exclusion)', !S.FIELD_SPECS.some((s) => s.name === 'Contract Seller Count'));
checkTrue('no checkbox marker key ("_mark") is part of this batch', !/_mark['"]/.test(src));
...
/* ==================================================================== */
/* 8. planBatch -- preflights ALL 4 specs before any POST; one conflict  */
/*    refuses the WHOLE batch; a future re-run after apply reuses all 4  */
...

{
  ...
  check('an all-clear preflight tags every one of the 4 specs "create"', result.plan.map((p) => p.action), new Array(4).fill('create'));
}
{
  const conflicting = realField({ name: S.FIELD_SPECS[3].name, fieldKey: 'opportunity.totally_wrong_key' }); // last spec
  const result = S.planBatch(S.FIELD_SPECS, [conflicting], PARENT_ID);
  checkTrue('ALL 4 specs are preflighted -- a conflict on spec #4 alone still surfaces', result.ok === false);
  check('exactly one conflict is reported, for the correct (last) key', result.conflicts.map((c) => c.key), [S.FIELD_SPECS[3].key]);
  check('every one of the 4 specs is still classified in the plan (full preflight, not stopped early)', result.plan.length, 4);
  checkTrue('field #1 is present in the plan even though the conflict is on field #4', result.plan[0].spec.key === S.FIELD_SPECS[0].key);
}
{
...
{
  // Simulates the state immediately after a future successful --apply:
  // all four fields now exist exactly as specified. Re-running the SAME
  // planBatch must classify every one "reuse", never "create" -- proving
  // a re-run can never produce a duplicate field.
  const allFour = S.FIELD_SPECS.map((spec, i) => realField({ id: `existing-id-${i + 1}`, name: spec.name, fieldKey: S.expectedFieldKey(spec.name) }));
  const result = S.planBatch(S.FIELD_SPECS, allFour, PARENT_ID);
  checkTrue('after a future successful apply, re-running planBatch is ok:true', result.ok === true);
  check('every one of the 4 specs classifies "reuse", never "create" -- no duplicate creation is possible', result.plan.map((p) => p.action), new Array(4).fill('reuse'));
}

/* ==================================================================== */
/* 9. NO --only bypass -- always the complete, unfiltered 4-spec batch   */
/* ==================================================================== */
...
/* ==================================================================== */
/* 13. This script does NOT modify the Seller Count script               */
/* ==================================================================== */

checkTrue('the Seller Count script file still exists, untouched by this batch', fs.existsSync(SELLER_COUNT_SCRIPT));
const sellerCountSrc = fs.readFileSync(SELLER_COUNT_SCRIPT, 'utf8');
checkTrue('the Seller Count script still targets contractSellerCount, not this batch\'s keys', /contractSellerCount/.test(sellerCountSrc) && !/additional_earnest_money_amount_text/.test(sellerCountSrc));
checkTrue('the Seller Count script still declares exactly 1 FIELD_SPEC, not 4', (sellerCountSrc.match(/key:\s*'contractSellerCount'/g) || []).length === 1);
