...
function verifyAgainstAuthoritativeSource() {
  ...
  if (authoritative.length !== 4) die(`authoritative CONTRACT_PROJECTION_TRANSPORT_ONLY_KEYS has ${authoritative.length} keys, expected 4 -- STOP, source has drifted since this script was written.`);
  if (JSON.stringify(authoritative) !== JSON.stringify(proposed)) {
    ...
    die(
      `FIELD_SPECS has drifted from CONTRACT_PROJECTION_TRANSPORT_ONLY_KEYS. missing=${JSON.stringify(missing)} extra=${JSON.stringify(extra)} -- ` +
      `refusing to run. Update this script's FIELD_SPECS to match the authoritative source exactly before retrying.`,
    );
  }
  console.log(`Verified: this script's 4 FIELD_SPECS keys match CONTRACT_PROJECTION_TRANSPORT_ONLY_KEYS exactly (same 4, same order).\n`);
}

/**
 * The ONE canonical anchor this script resolves its target folder from --
 * identical discipline to every prior batch: the anchor must exist exactly
 * once and carry a non-empty `parentId`, or this script refuses before any
 ...
 * identical logic to every prior batch. `{kind:'none'}` (no match, safe to
 * create), `{kind:'exact_existing', field}` (name AND fieldKey AND
 * dataType AND model AND parentId all agree -- safe to reuse, after
 * independent re-verification), or `{kind:'conflict', reasons, field?}` (a
 * partial match, multiple matching records, or a full identity match whose
 ...
 * every prior batch: `model` and `parentId` are checked strictly.
 */
...
/**
 * Parses a POST response body defensively -- identical to every prior
 * batch's `parsePostResponse`. A parse failure or a missing id means GHL's
 * own confirmation of what (if anything) was created cannot be trusted --
