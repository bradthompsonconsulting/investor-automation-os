/* PB-D60 -- opportunity.asking_price inert proof, step 2 WRITE.
 * ONE PUT. NO RE-READ, NO POLL.
 *
 * One custom-fields-only PUT setting the designated test value 642318.57,
 * approved before the write by PB-D60 under PB-D30's 2026-08-03 amendment.
 * DELIBERATELY SELECTED, never described as observed.
 *
 * ⚠ THE VALUE IS JUSTIFIED ON THE WIRE, NOT ON SCREEN. PB-D60 records the
 * correction: RAIL_MONEY formats with maximumFractionDigits: 0, so this value
 * renders in the rail as "$642,319" -- a plausible ask. Almost any six-figure
 * value is, so operator exposure is controlled by the bounded proof window,
 * not by the choice of number. The value carries wire and evidence
 * identification only, and is distinct from all ten values already in use.
 *
 * ⚠ THE BODY IS CUSTOM-FIELDS-ONLY. PB-D58 section II: "No pipelineStageId,
 * no status, no name, no monetaryValue in any PUT body... That is the
 * mechanism the whole proof rests on, and a PUT body carrying anything else
 * forfeits it."
 *
 * ⚠ AFTER THIS STEP THE FIXTURE CARRIES A NON-REAL ASKING PRICE, visible in
 * the rail and active in Underwriting. Restoration is not optional cleanup;
 * it is what completes the proof.
 *
...
const EVIDENCE = "C:/Users/brad/AppData/Local/Temp/inert-proof-opp-asking-price-step2.json";
...
(async () => {
  ...
  if (cap.fieldPresent !== false) {
    fail(114, `step 1 recorded fieldPresent=${cap.fieldPresent}; the absent-origin contract requires an absent origin`);
  }
  ...
  const record = {
    ...
    stage: "write",
    cycle: "proof",
    note: "PB-D60 field-safety proof for opportunity.asking_price. Does NOT discharge PB-D56 prerequisite 5; that was discharged by PB-D58 section VI on a different field.",
