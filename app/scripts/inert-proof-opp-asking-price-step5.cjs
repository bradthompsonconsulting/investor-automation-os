/* PB-D60 -- opportunity.asking_price inert proof, step 5 CONFIRM.
 * READ ONLY. NO WRITES.
 *
 * The step that establishes field safety for opportunity.asking_price, or
 * does not.
 *
 * ⚠ WIRE SHAPE CONSUMED: the SINGULAR GET /opportunities/{id}. ABSENCE IS
 * DECIDED STRUCTURALLY -- `customFields.find(f => f.id === TARGET_ID)`
 * returning null, i.e. the id is gone from the array -- and NEVER by a parser
 * returning undefined. That distinction is the whole protection against the
 * singular/list shape hazard, because a mis-shaped parser cannot manufacture
 * absence out of a key that is present.
 *
 ...
 *
 * restoredToOrigin is stricter than CLEARED: the field must be absent AND the
 * rest of the record must be where capture left it. Absence alone is not
 * restoration.
 *
...
const STEP3    = "C:/Users/brad/AppData/Local/Temp/inert-proof-opp-asking-price-step3.json";
const STEP4    = "C:/Users/brad/AppData/Local/Temp/inert-proof-opp-asking-price-step4.json";
const EVIDENCE = "C:/Users/brad/AppData/Local/Temp/inert-proof-opp-asking-price-step5.json";
...
(async () => {
  ...
  /* INCIDENTAL PROTECTION — NOT the provenance mechanism, and NOT coverage.
     ...
     not an abstraction — it is the provenance of the FIELD-SAFETY record for
     opportunity.asking_price. It is NOT a discharge record; PB-D58 section VI
     holds the prerequisite-5 discharge, on a different field.
  ...
  /* PB-D58 section IV's SEVEN CONDITIONS, reused here as the definition of a
     completed absent-origin proof and asserted from evidence rather than
     memory. ⚠ THEY ARE NOT A PREREQUISITE-5 DISCHARGE IN THIS SUITE. PB-D58
  ...
  const fieldSafetyEstablished = Object.values(fieldSafety).every(Boolean);
  ...
  const record = {
    ...
    stage: "confirm",
    cycle: "proof",
    note: "PB-D60 field-safety proof, confirm stage. This record is the FIELD-SAFETY evidence for opportunity.asking_price. It is NOT prerequisite-5 discharge evidence; PB-D58 section VI holds that, on a different field.",
    ...
    restoredToOrigin,
    /* RENAMED FROM discharge/dischargeable. The inherited key names were
       themselves an emitted false claim: a reader parsing this artifact would
       see "discharge": {...} and reasonably read a prerequisite-5 discharge.
       These seven conditions are PB-D58 section IV's, reused here as the
       definition of a completed absent-origin proof, and what they establish
       is FIELD SAFETY for one field. */
    fieldSafety,
    fieldSafetyEstablished,
    drifted,
    offerNowPresent,
  ...
  console.log("");
  console.log("PB-D58 section IV conditions, applied here as the field-safety definition:");
  for (const [k, v] of Object.entries(fieldSafety)) {
    console.log(`  ${v ? "MET    " : "NOT MET"}  ${k}`);
  }
  console.log("");

  if (outcome === "CLEARED" && restoredToOrigin && fieldSafetyEstablished) {
    console.log("  All seven conditions MET.");
    console.log("  FIELD SAFETY for opportunity.asking_price is established by this evidence.");
    console.log("  This does NOT discharge PB-D56 prerequisite 5 — PB-D58 section VI already did, on a different field.");
    console.log("  Nor does field safety authorize Approve: PB-D58 section IV holds that even a");
    console.log("  discharge does not, and this is less than a discharge.");
    process.exit(0);
  }
  if (outcome === "CLEARED" && !restoredToOrigin) {
    console.log("  The key is absent BUT the record is not where capture left it.");
    console.log("  The restore was not inert. Field safety is NOT established.");
    process.exit(203);
  }
  console.log(`  ${ev.s4.mechanism} did NOT return the field to absent. Outcome: ${outcome}.`);
  console.log("  The field is left populated and field safety is NOT established. ⚠ RESTORATION REQUIRED — PB-D60 makes this an immediate STOP, not an ordinary failure.");
