...
export function computeRepairEstimate(
  ...
  // The inherited factor applies only to resolved BOOK-derived amounts.
  // IAOS POLICY reserves and MANUAL amounts remain part of the estimate,
  // but neither is authority for this historical cost-book allowance.
  const fmtmBasis = byProvenance.BOOK;
  const fmtmAllowance: FmtmAllowance = {
    label: FMTM_ALLOWANCE_LABEL,
    ratePct: 10,
    outcome: { kind: "priced", basis: fmtmBasis, amount: fmtmBasis * 0.1 },
  };
